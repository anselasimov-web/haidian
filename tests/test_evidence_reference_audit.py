...
class EvidenceReferenceAuditTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.write_json(self.package / "sources.json", {"sources": [{"source_id": "SRC-LOCAL"}]})
        self.write_json(self.repo / "data" / "source_registry.json", {"sources": [{"source_id": "SRC-CENTRAL"}]})
        self.write_json(self.repo / "sources" / "public-sources.json", {"sources": [{"id": "SRC-PUBLIC"}]})
        self.write_json(self.package / "standard_matrix.json", {"standards": [{"standard_id": "STD-PACKAGE"}]})
        self.write_json(
            self.repo / "brief" / "site-package" / "standards" / "standards.json",
            {"standards": [{"standard_id": "STD-CENTRAL"}]},
        )
        self.write_json(self.package / "design_depth_matrix.json", {"items": [{"item_id": "depth_item"}]})
        self.write_json(
        ...
        self.write_json(self.repo / "brief" / "site-package" / "ranges" / "planning_limits.json", {"status": "missing"})
    ...
    def test_reports_dangling_ids_and_feature_fragment(self) -> None:
        ...
        self.assertIn("feature ID does not resolve", next(item for item in findings if item.kind == "data").reason)

    def test_missing_data_file_and_escaping_path_are_reported(self) -> None:
        proposal = self.write_proposal("[data:geometry/missing.geojson#X] [data:../../outside.geojson#X]")
        findings = AUDIT.audit_proposal(self.repo, proposal)
    ...
    def test_translation_files_are_discovered_for_package_target(self) -> None:
        ...
        self.assertEqual(AUDIT.proposal_paths(self.repo, str(self.package), False), [primary, translation])
